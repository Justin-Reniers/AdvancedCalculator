...
class Addition(BinaryExpression):
    ...
    def optimize(self):
        ...
        if isinstance(left, Constant) and left.get_constant() == 0:
            return right
        if isinstance(right, Constant) and right.get_constant() == 0:
            return left
        if isinstance(right, Constant) and right.get_constant() == 0 and \
                isinstance(left, Constant) and left.get_constant() == 0:
            return Constant(0)
        return self
